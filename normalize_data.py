...
for file in files:
    ...
    for prod in products:
        row = {
            "name": None, "category": None, "brand": None, "strainType": None, "priceRec": None, "dispensary": file.split('_')[0], "productUrl": None, "description": None
        }
        # --- brotherlybud ---
        if "brotherlybud" in file:
            ...
            row["price"] = v.get("specialPriceRec") if v.get("specialPriceRec") is not None else v.get("priceRec")
            row["productUrl"] = f"https://brotherlybud.com/product/{prod.get('slug')}" if prod.get('slug') else None
            if row["productUrl"]:
                row["productUrl"] += "/?retailer_id=14beadc1-5ad4-49df-b412-7fe5118669cc"
        ...
        # --- curaleaf ---
          ...
          row["brand"] = (prod.get("brand") or {}).get("name")
          strain_type = (prod.get("strain") or {}).get("prevalence", {}).get("name") or (prod.get("strain") or {}).get("name") or ""
          row["strainType"] = strain_type
          v = prod.get("variants", [{}])[0]
          row["price"] = v.get("promoPrice") if v.get("promoPrice") is not None else v.get("price")
          ...
          def slugify(val):
              ...
              return val.strip("-")
          if all([category_name, category_id, subcategory_name, strain_type, row["name"], v.get("name"), v.get("id")]):
        ...
        # --- indigodispensary ---
            ...
            row["category"] = v.get("productCategoryName") or prod.get("productCategoryName")
            row["brand"] = (v.get("brand") or {}).get("name") or (prod.get("brand") or {}).get("name")
        ...
        # --- mpxnj ---
            ...
            row["price"] = v.get("specialPriceRec") if v.get("specialPriceRec") is not None else v.get("priceRec")
            if prod.get('slug'):
                row["productUrl"] = f"https://mpxnj.com/product/{prod.get('slug')}/?retailer_id=11883dc4-d7d7-4085-8e7b-ba5353eca58a"
            else:
                row["productUrl"] = None
            row["description"] = prod.get("description")
        # --- shopcuzzies ---
            ...
            try:
                row["price"] = float(v.get("specialPrice")) if v.get("specialPrice") else float(v.get("price"))
            except Exception:
                row["price"] = None
            row["productUrl"] = f"https://www.shopcuzzies.com/?product={prod.get('jointId')}" if prod.get('jointId') else None
            row["description"] = prod.get("description")
...
for row in rows:
    ...
    quant_match = re.search(r'(\d+)\s?(\w+\s?){0,2}?-?(?:p(?:cs?|ieces?)|pks?|per package|per bag|packs?|ct)\b', text, re.IGNORECASE)
