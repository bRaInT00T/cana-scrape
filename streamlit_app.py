...
selected_strain = st.sidebar.multiselect(
    "Strain",
    sorted(strains),
    default=["Hybrid", "Indica", "Hybrid/Indica", "Hybrid/Sativa"],
)
# Dispensary filter
...
if "price" in filtered.columns and not filtered.empty:
    min_price, max_price = float(filtered["price"].min()), float(
        filtered["price"].max()
    )
...
filtered_cols = [
    col for col in desired_order if col in filtered.columns and col not in hide_columns
]
    ...
    st.dataframe(
        ...
        column_config={
            "Link": st.column_config.LinkColumn(
                label="Link", display_text="🔗", max_chars=None
            ),
        },
    )
else:
    st.dataframe(
        filtered.sort_values(["Price/mg", "Price"]),
        use_container_width=True,
        hide_index=True,
    )
