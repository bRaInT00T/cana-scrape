...
if "specialPriceRec" in df.columns:
    df["price"] = df.apply(
        lambda row: (
            row["specialPriceRec"]
            if pd.notnull(row.get("specialPriceRec"))
            else row.get("priceRec")
        ),
        axis=1,
    )
else:
...
df["productUrl"] = (
    "https://mpxnj.com/product/"
    + df["slug"]
    + "/?retailer_id=11883dc4-d7d7-4085-8e7b-ba5353eca58a"
)
...
filtered_df = (
    df[df["type"].isin(selected_categories)] if selected_categories else df.copy()
)
filtered_df = (
